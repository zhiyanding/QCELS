...
def generate_data_set(x,t,d,delta,spectrum,population,Nsample,Nbatch):
...
def generate_spectrum_population(eigenenergies, population, p0,p1):
    spectrum = eigenenergies * 0.25*np.pi/np.max(np.abs(eigenenergies))
    q = population
    q[0:2] = np.array([p0,p1])/(1-p0-p1)*np.sum(q[2:])
    return spectrum, q/np.sum(q)

def qcels_opt_fun(x, ts, ovlp):
    NT = ts.shape[0]
    signal=np.zeros(NT,dtype = 'complex_')
    for i in range(NT):
        signal=(x[0]+1j*x[1])*np.exp(-1j*x[2]*ts)
    return (np.linalg.norm(signal-ovlp)/NT)

def qcels_opt(ts, ovlp, x0, bounds = None, method = 'SLSQP'):

    fun = lambda x: qcels_opt_fun(x, ts, ovlp)
    if( bounds ):
...
def qcels_filtered(spectrum, population, T, NT, d, rel_gap, err_tol_rough, Nsample_rough, Nsample):
    """QCELS with a filtered data set for small initial population.
    ...
    ts=tau*np.arange(NT)
    for i in range(NT):
        ovlp[i], total_time, max_time=generate_data_set(
                x,ts[i],d,err_tol_rough,spectrum,population,Nsample_rough,Nbatch=1)
        total_time_all += total_time
        max_time_all = max(max_time_all, max_time)
    x0=np.array((0.5,0,lambda_prior))
    res = qcels_opt(ts, ovlp, x0)
    ...
    for n_QCELS in range(N):
        ...
        for i in range(NT):
            ovlp[i], total_time, max_time=generate_data_set(
                    x,ts[i],d,err_tol_rough,spectrum,population,Nsample,Nbatch=1)
            total_time_all += total_time
            max_time_all = max(max_time_all, max_time)
        ...
        res = qcels_opt(ts, ovlp, x0, bounds=bnds)
        ground_coefficient_QCELS=res.x[0]
        ground_coefficient_QCELS2=res.x[1]
        ground_energy_estimate_QCELS=res.x[2]
        lambda_min=ground_energy_estimate_QCELS-np.pi/(2*tau)
...
if __name__ == "__main__":
    ...
    for a1 in range(len(p0_array)):
        ...
        for n_test in range(Navg):
            ...
            Nsample_rough=int(200/p0**2*np.log(d))
            Nsample=int(12/p0**2*np.log(d))
            for ix in range(len(T_list_QCELS)):
                ...
                ground_energy_estimate_QCELS, cost_list_avg_QCELS[a1,ix], max_T_QCELS[a1,ix] = \
                        qcels_filtered(spectrum, population, T, NT, d, rel_gap, \
